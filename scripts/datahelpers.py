...
def get_all_nap_data(with_upsample=False):
    ...
    create_URL = lambda row: url_for(
        "static", filename="ReadingImagesResized/" + row["file_name"]
    )
...
def aggregate_nap_data(df):
    ...
    napsDF["start_date_string"] = napsDF.apply(
        lambda row: today_or_yesterday(row["start_time"]), axis=1
    )
    napsDF["start_time_string"] = napsDF.apply(
        lambda row: time_string(row["start_time"]), axis=1
    )
    ...
    napsDF["end_time_string"] = napsDF.apply(
        lambda row: (
            time_string(row["end_time"]) if not is_NaT(row["end_time"]) else "Now"
        ),
        axis=1,
    )
    ...
    napsDF["duration"] = napsDF.apply(
        ...
        lambda row: (
            get_duration(row)
            if not is_NaT(row["end_time"])
            else (convert_timezone(demo_date) - row["start_time"])
        ),  # Sleep is ongoing, calculate current duration
        axis=1,
    )

    napsDF["duration_string"] = napsDF.apply(
        lambda row: duration_to_string(
            time_delt=row["duration"], ongoing=row["end_time_string"] == "Now"
        ),
        axis=1,
    )
...
def get_training_data():
    ...
    df["folder_resized"] = df.apply(
        lambda row: get_folder_name(row["ResizedPath"]), axis=1
    )
    df["folder_original"] = df.apply(
        lambda row: get_folder_name(row["FilePath"]), axis=1
    )
...
def get_readings(as_df=False, with_rendered_values=True, date_based=True):
    ...
    if date_based:
        query = (
            db.session.query(DataPoint)
            .filter(DataPoint.timestamp >= demo_date - timedelta(days=7))
            .filter(DataPoint.timestamp <= demo_date)
        )
        reverse = False
    else:
        query = (
            db.session.query(DataPoint).order_by(DataPoint.timestamp.desc()).limit(100)
        )
        reverse = True
    ...
    df["file_name"] = df.apply(
        lambda row: get_file_name(row["image_orig_path"]), axis=1
    )
    ...
    if with_rendered_values:
        for field in ["baby_reading", "empty_reading"]:
            df[field] = df.apply(
                lambda row: round_number_for_display(row[field]), axis=1
            )
    if as_df:
        return df
    else:
...
def get_bedtime_data(start_id, end_id):
    ...
    query = db.session.query(DataPoint).filter(DataPoint.id.between(start_id, end_id))
    ...
    df["file_name"] = df.apply(
        lambda row: get_file_name(row["image_resized_path"]), axis=1
    )
    ...
    duration_string = duration_to_string(
        time_delt=duration, ongoing=time_string_last == "Now"
    )
...
def duration_to_string(time_delt, ongoing=False):
    ...
    if hours:
        returned_string += str(hours) + f" hour{plural_or_not(hours)}"
        if minutes:
            returned_string += ", "
    if minutes:
        returned_string += (
            str(minutes) + f" minute{plural_or_not(minutes)}{so_far(ongoing)}"
        )
...
def get_model_results(predictions_csv):
    ...
    create_URL = lambda row: url_for(
        "static", filename=row["folder_resized"] + "/" + row["file_name"]
    )
    df["photo_url"] = df.apply(create_URL, axis=1)

    babyDF = df.loc[
        (df["Value"] == "Baby") | (df["Value"] == 1.0) | (df["Value"] == 2.0)
    ].reset_index()
    baby_likeliness_binary = (
        lambda row: 1 * ((row["LikelyBaby"] - row["LikelyEmpty"]) > 0) - 0.5
    )
    ...
    label_dict = {
        "baby_incorrect": str(baby_incorrect),
...
def today_or_yesterday(date_obj):
    ...
    elif obj_date_string == date_string(
        convert_timezone(demo_date - timedelta(hours=24))
    ):
...
def paginate(list_to_pag, items_per_pag, page_requested):
    ...
    if page_requested <= pagination_rad:
        ...
        pagination = [
            i
            for i in range(1, pages + 1)
            if abs(page_requested - i)
            < abs(2 * pagination_rad - (pages - page_requested + 1))
        ]
