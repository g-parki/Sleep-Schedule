...
def live_prediction_graph(data_source):
    ...
    # Labels
    left_annotation = Label(
        x=-0.93, y=-0.065, text="Empty", text_align="left", text_font_size="12px"
    )
    center_annotation = Label(
        x=0,
        y=-0.065,
    ...
    right_annotation = Label(
        x=0.93, y=-0.065, text="Baby", text_align="right", text_font_size="12px"
    )
    ...
    p.scatter("x", "y", source=data_source, color="black", size=10, fill_alpha=0.3)
...
def bedtime_graph(sourceDF, fillsourceDF):
    ...
    source = ColumnDataSource(
        data=dict(
            ...
            value=0.5 * (sourceDF.apply(lambda row: row["value"] == "Baby", axis=1))
            + 0.25,
    ...
    fillsource = ColumnDataSource(
        data=dict(
            date=convert_timezone_np(fillsourceDF.index),
            value=0.5 * (fillsourceDF.apply(lambda row: row["value"] == "Baby", axis=1))
            + 0.25,
        )
    )
    ...
    p = figure(
        ...
        x_range=Range1d(
            start=convert_timezone(demo_date - timedelta(hours=24)),
        ...
        y_range=Range1d(start=-0.07, end=1.07, bounds=(-0.07, 1.07)),
        plot_height=300,
    ...
    select.varea(source=fillsource, x="date", y1=0.25, y2="value", alpha=0.2)
...
def model_performance_graph_quads(baby_df, nobaby_df, dims):
    ...
    JITTER_RADIUS_X = 0.97
    JITTER_RADIUS_Y = 0.47
    DOT_SIZE = 10
    DOT_ALPHA = 0.1
    BACK_ALPHA = 0.05
    ...
    p = figure(
        ...
        x_axis_location="above",
        x_range=Range1d(
            start=dims.get("x_start"),
            end=dims.get("x_end"),
            bounds=(dims.get("x_start"), dims.get("x_end")),
        ),
        y_range=Range1d(
            start=dims.get("y_start"),
            end=dims.get("y_end"),
            bounds=(dims.get("y_start"), dims.get("y_end")),
        ),
        plot_height=200,
    )
    ...
    p.line([-1.5, 1.5], [0.5, 0.5], line_width=1, color="dimgray")  # horizontal
