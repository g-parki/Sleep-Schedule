...
def home():
    ...
    return render_template(
        ...
        graph_script=script,
        graph_div=div,
        rendered_table=render_template("subtemplates/bedtimestable.html", data=data),
    )
...
def refresh_bedtimes():
    ...
    return render_template("subtemplates/bedtimestable.html", data=data)
...
def bedtime():
    ...
    data, date_string, time_string_first, time_string_last, duration_string = (
        datahelpers.get_bedtime_data(start_id, end_id)
    )
...
def video_feed(
    inqueue=classification_q, outqueue=class_success_q, event=class_success_ev
):
    """Video streaming route. Put this in the src attribute of an img tag."""

    return Response(
        stream_with_context(iter(streamer.Streamer(inqueue, outqueue, event))),
...
def classify(
    outqueue=classification_q, inqueue=class_success_q, event=class_success_ev
):
...
def livefeed():
    ...
    ajax_source = AjaxDataSource(
        data_url=url_for("ajaxprediction"), polling_interval=1000, mode="replace"
    )
    ajax_source.data = dict(x=[], y=[])

    script, div = graphs.live_prediction_graph(ajax_source)

    return render_template("livefeed.html", graph_div=div, graph_script=script)
...
def correct_training_data():
    ...
    if request.method == "POST":
        ...
        response = datahelpers.update_training_csv(
            filename=data["file_name"], new_value=data["value"]
        )
...
def models():
    ...
    model_list = os.listdir(
        os.path.join(os.getcwd(), "scripts", "static", "modelpredictions")
    )
    model_list.reverse()
...
def model(modelname):
    ...
    model_folder_path = os.path.join(
        os.getcwd(), "scripts", "static", "modelpredictions", modelname
    )
    ...
    babyDF, nobabyDF, accuracy, label_dict = datahelpers.get_model_results(
    ...
    for quad in quadrant_dims:
        quad["script"], quad["div"] = graphs.model_performance_graph_quads(
            babyDF, nobabyDF, quad
        )
