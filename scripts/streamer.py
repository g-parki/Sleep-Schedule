...
def predictor(frame, model):
    ...
    if frame.empty_prediction > frame.baby_prediction:
        frame.prediction_str = (
            f"PREDICTION: EMPTY BED ({frame.empty_prediction: .6f}, "
            f"{frame.baby_prediction: .6f})"
        )
    else:
        frame.prediction_str = (
            f"PREDICTION: BABY IN BED ({frame.empty_prediction: .6f}, "
            f"{frame.baby_prediction: .6f})"
        )

    frame.prediction_image = cv2.putText(
        img=frame.mediumsize,
    ...
    frame.prediction_image_en = cv2.imencode(".jpg", frame.prediction_image)[
        1
    ].tobytes()
    return frame
...
def refresh_stream_token():
    ...
    access_token = refresh_access_token(
        "./TokensAndResponses/client_secret.json",
    ...
    original_data["results"]["streamExtensionToken"] = json_response["results"][
...
def pacific_datetime(rfc_string):
    ...
    return (
        old_timezone.localize(gmt_expiration_date)
...
def get_stream_URL():
    ...
    with open("./TokensAndResponses/streaminfo.json", "r") as f:
        ...
        try:
            ...
            previous_expiration_localized = pacific_datetime(
                previous_expiration_rfc_string
            )

            # Previous URL still valid for at least another minute
            if previous_expiration_localized > datetime.now() + timedelta(minutes=1):
                print(
                    f"Previous token still valid, expires at {previous_expiration_localized}"
                )
...
def refresh_stream_checker(expires_in_seconds):
    ...
    if current_stream_expiration_time < datetime.now() + timedelta(
        seconds=expires_in_seconds
    ) and "Refresh Thread" not in [thread.name for thread in enumerate()]:
        t = Thread(
            name="Refresh Thread", target=refresh_stream_token, args=[], daemon=True
        )
        t.start()
    ...
    def __iter__(self):
        ...
        while True:
            ...
            if not self.inqueue.empty():
                response = training_data_saver(
                    frame=self.frame, value=self.inqueue.get()
                )
                if response:
            ...
            yield (
                ...
                + self.frame.prediction_image_en
                + b"\r\n"
            )
    ...
    def run_local_display(self):
        ...
        while True:
            ...
            if self.key in [ord("0"), ord("1"), ord("2")]:
                response = training_data_saver(
                    frame=self.frame, value=format(int(chr(self.key)), ".1f")
                )
                if response:
                    print(response)

            # close window
            if (self.key == ord("q")) or cv2.getWindowProperty(
